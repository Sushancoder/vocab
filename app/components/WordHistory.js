...
export default function WordHistory({
  ...
  return (
    <div
      className="text-xl font-bold text-center text-gray-600 mb-8 relative rounded-2xl p-2 cursor-pointer shadow-md shadow-blue-300 group"
    ...
    >
      ...
      <div
        className={`absolute top-10 right-0 rounded-lg text-black bg-gray-200 min-w-64 z-30 border-2 border-blue-400 shadow-md shadow-blue-300 transform transition-all duration-300 ease-in-out origin-top-right ${showWords ? "opacity-100 scale-100" : "opacity-0 scale-90 pointer-events-none"} ${isSending ? "opacity-0 cursor-not-allowed" : ""}`}
      >
        {usedWords.length > 0 ? (
          <>
            <div className="p-3 bg-white shadow-gray-400 rounded-t-lg text-xs font-thin text-black cursor-default">
              Click the word to know it's details or the delete button to delete
              it.
            </div>
        ...
        <div
          className={`overflow-x-hidden max-h-[48vh] rounded-b-lg ${isSending ? "overflow-hidden" : "overflow-auto"} custom-scrollbar`}
        >
